// dsh-theme-background client 半：浏览器端 classic-script bundle。
// 由 dsh-client-modules 以 /plugins/dsh-theme-background/client.js 提供给浏览器，
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var STORAGE_KEY = "dsh-theme-background:image";
    var SETTINGS_KEY = "dsh-theme-background:settings";
    var BACKGROUND_TAG_ID = "dsh-theme-background/background.css";
    var ROW_TAG_ID = "dsh-theme-background/BackgroundRow.css";
    var LAYER_ID = "dsh-theme-background-layer";
    var MAX_DIM = 1920;
    ...
    if (typeof document !== "undefined" && document.querySelector('style[data-plugin-css="' + ROW_TAG_ID + '"]') === null) {
      ...
      rowTag.dataset.plugin = "dsh-theme-background";
    ...
    function saveImage(value) {
      try {
        ...
        try {
          console.warn("[dsh-theme-background] 保存背景失败（可能超出 localStorage 配额）", e);
        } catch (_) {}
      }
    }
    ...
    function saveSettings(settings) {
      try {
        ...
        try {
          console.warn("[dsh-theme-background] 保存清晰度/遮罩设置失败", e);
        } catch (_) {}
      }
    }
    ...
    function apply(ctx) {
      ...
      function ensureLayer() {
        if (!layerEl) {
          ...
          layerEl.id = LAYER_ID;
          layerEl.dataset.plugin = "dsh-theme-background";
      ...
      function applyVisual() {
        ...
        if (!cssTag) {
          ...
          cssTag.dataset.plugin = "dsh-theme-background";
      ...
      }, "dsh-theme-background: background css");
